...
let enabledServices = Object.keys(s)
  .filter((p) => s[p].enabled)
  .sort()
  .map((p) => {
...
<html lang="${obj.lang}">
    <head>
        <meta charset="utf-8">
        <meta name="viewport" content="viewport-fit=cover, width=device-width, height=device-height, initial-scale=1, maximum-scale=${
          isIOS ? `1` : `5`
        }">
    ...
    <body id="cobalt-body" ${platform === "d" ? 'class="desktop"' : ""}>
          ...
          tabs: [
                    ...
                    text: collapsibleList([
                      {
                        ...
                        body:
                          `${enabledServices}` +
                          `<div class="explanation embedded">${t(
                            "SupportNotAffiliated"
                          )}` +
                          `${
                            obj.lang === "ru"
                              ? `<br>${t("SupportMetaNoticeRU")}`
                              : ""
                          }` +
                          `</div>` +
                          `${t("ServicesNote")}`,
            ...
            },
            {
              ...
              content: popup({
                ...
                body: [
                  {
                    text: `<div class="category-title">${t(
                      "ChangelogLastMajor"
                    )}</div>`,
                    raw: true,
                  },
                  ...
                  },
                  {
                    text: `
                        <div id="changelog-history">
                            <button class="switch bottom-margin" onclick="loadOnDemand('changelog-history', '0')">${t(
                              "ChangelogPressToExpand"
                            )}</button>
                        </div>`,
                    raw: true,
                  },
          ...
          tabs: [
            ...
            },
            {
              ...
              content:
                ...
                settingsCategory({
                  name: "language",
                  title: t("SettingsLanguageSubtitle"),
                  body: switcher({
                    name: "language",
                    items: [
                      {
                        action: "auto",
                        text: t("SettingsLanguageAuto"),
                      },
                      {
                        action: "english",
                        text: t("SettingsLanguageEnglish"),
                      },
                      {
                        action: "russian",
                        text: t("SettingsLanguageRussian"),
                      },
                    ],
                  }),
                }) +
        ...
        ${popupWithBottomButtons({
          ...
          buttons: [
            `<a id="picker-download" class="switch" target="_blank" href="/">${t(
        ...
        <div id="popup-download-container" class="popup-from-bottom">
            ${popup({
              ...
              body: switcher({
                ...
                explanation: `${
                  !isIOS
                    ? t("DownloadPopupDescription")
                    : t("DownloadPopupDescriptionIOS")
                }`,
                items: `<a id="pd-download" class="switch full" target="_blank" href="/"><span>${t(
                  "Download"
                )}</span></a>
                    <div id="pd-share" class="switch full">${t(
                      "ShareURL"
                    )}</div>
                    <div id="pd-copy" class="switch full">${t(
                      "CopyURL"
                    )}</div>`,
              }),
            ...
            <div id="cobalt-main-box" class="center">
                ...
                <div id="download-area">
                    <div id="top">
                        ...
                        <input id="url-input-area" class="mono" type="text" autocomplete="off" spellcheck="false" maxlength="256" autocapitalize="off" placeholder="${t(
                          "LinkInput"
                        )}" aria-label="${t(
      "AccessibilityInputArea"
    )}" oninput="button()">
                        <button id="url-clear" onclick="clearInput()" style="display:none;">x</button>
                        <input id="download-button" class="mono dontRead" onclick="download(document.getElementById('url-input-area').value)" type="submit" value="" disabled aria-label="${t(
                          "AccessibilityDownloadButton"
                        )}">
                    </div>
                    <div id="bottom">
                        <button id="paste" class="switch" onclick="pasteClipboard()" aria-label="${t(
                          "PasteFromClipboard"
                        )}">${emoji("📋", 22)} ${t(
      "PasteFromClipboard"
    )}</button>
